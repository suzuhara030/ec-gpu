...
DEVICE POINT_projective POINT_double(POINT_projective inp) {
  ...
  inp.z = FIELD_mul(inp.y, inp.z);
  inp.z = FIELD_double(inp.z);           // Z3 = 2*Y1*Z1
...
DEVICE POINT_projective POINT_add_mixed(POINT_projective a, POINT_affine b) {
  ...
  FIELD i = FIELD_double(hh);
  i = FIELD_double(i);       // I = 4*HH
  FIELD j = FIELD_mul(h, i); // J = H*I
  FIELD r = FIELD_sub(s2, a.y);
  r = FIELD_double(r); // r = 2*(S2-Y1)
  ...
  // Z3 = (Z1+H)^2-Z1Z1-HH
  ret.z = FIELD_add(a.z, h);
  ret.z = FIELD_sub(FIELD_sub(FIELD_sqr(ret.z), z1z1), hh);
  return ret;
}
