...
DEVICE FIELD FIELD_add(FIELD a, FIELD b) {
  ...
  if (FIELD_gte(res, FIELD_P))
    res = FIELD_sub_(res, FIELD_P);
  return res;
}
...
DEVICE void FIELD_reduce(uint32_t accLow[FIELD_LIMBS], uint32_t np0,
                         uint32_t fq[FIELD_LIMBS]) {
...
#pragma unroll
      ...
      for (i = 0; i < count - 2; i += 2) {
        accHigh[i] = chain_madlo(&chain2, q, fq[i + 1],
                                 accHigh[i + 2]); // note the shift down
...
#pragma unroll
      ...
      chain_init(&chain4);
      for (i = 0; i < count - 2; i += 2) {
        accLow[i] = chain_madlo(&chain4, q, fq[i + 1],
                                accLow[i + 2]); // note the shift down
...
DEVICE inline void FIELD_mult_v1(uint32_t *x, uint32_t *y, uint32_t *xy) {
...
#ifdef CUDA
DEVICE FIELD FIELD_mul(FIELD a, FIELD b) { return FIELD_mul_nvidia(a, b); }
#else
DEVICE FIELD FIELD_mul(FIELD a, FIELD b) { return FIELD_mul_default(a, b); }
#endif

// Squaring is a special case of multiplication which can be done ~1.5x faster.
// https://stackoverflow.com/a/16388571/1348497
DEVICE FIELD FIELD_sqr(FIELD a) { return FIELD_mul(a, a); }
...
DEVICE FIELD FIELD_mont(FIELD a) { return FIELD_mul(a, FIELD_R2); }
...
DEVICE bool FIELD_get_bit(FIELD l, uint i) {
  return (l.val[FIELD_LIMBS - 1 - i / FIELD_LIMB_BITS] >>
          (FIELD_LIMB_BITS - 1 - (i % FIELD_LIMB_BITS))) &
         1;
}
